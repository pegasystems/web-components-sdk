...
class FlowContainer extends BridgeBase {
  ...
  updateSelf() {
    ...
    if (this.caseMessages || !this.hasAssignments()) {
      ...
      this.checkSvg = Utils.getImageSrc(
        "check",
    ...
    if (routingInfo && loadingInfo != undefined) {
      if (this.bLogging) {
        console.log(
          `${this.theComponentName}: >>routingInfo: JSON.stringify(routingInfo)`,
        );
      }
      ...
      if (currentOrder && currentItems) {
        ...
        if (currentOrder.length > 0) {
          if (
            ...
            if (oWorkData) {
              this.containerName =
                this.getActiveViewLabel() ||
                oWorkData.caseInfo.assignments[0].name;
              this.instructionText =
                oWorkData.caseInfo.assignments[0].instructions;
            }
  ...
  flowContainerHtml(): any {
    const fCHtml = html` <div
      style="text-align: left;"
      id="${this.buildName}"
      class="psdk-flow-container-top"
    >
        ...
        ${
          !this.todo_showTodo
            ? html`
                ...
                ${this.instructionText !== ""
                  ? html`<div class="psdk-instruction-text">
                      ${this.instructionText}
                    </div>`
                  : nothing}
                <div>
