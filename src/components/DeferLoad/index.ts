...
class DeferLoad extends BridgeBase {
  ...
  loadActiveTab(data: any = {}) {
    if (this.bLogging) {
      console.log(
        `DeferLoad: loadActiveTab data: ${JSON.stringify(data)} | loadData: ${JSON.stringify(this.loadData)}`,
      );
    }
      ...
      loadView(
        ...
        .then((data) => {
          ...
          if (this.componentName === "reference") {
            this.loadedPConn = this.loadedPConn
              .getReferencedViewPConnect(true)
              .getPConnect();
  ...
  getDeferLoadHtml(): any {
    ...
    switch (this.componentName) {
      case "View":
        arComponent.push(
          html`<view-component .pConn=${this.loadedPConn}></view-component>`,
        );
        break;
      ...
      default:
        arComponent.push(
          html`<div>Defer load missing: ${this.componentName}</div>`,
        );
        break;
    }
