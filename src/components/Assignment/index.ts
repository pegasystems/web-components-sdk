...
class Assignment extends BridgeBase {
  ...
  // buttons
  arMainButtons: Array<unknown> = new Array();
  ...
  renderer: NotificationLitRenderer = () => {
    return html`
      <div style="align-items: center; display: flex">
        ...
        <button
          style="background: none; border: none; margin: 2px 0px 0px 8px; cursor: pointer;"
          @click=${this.close}
        >
          X
        </button>
    ...
    if (oWorkData) {
      ...
      if (oWorkData.caseInfo && oWorkData.caseInfo.assignments != null) {
        ...
        if (oCaseInfo.navigation != null) {
          ...
          this.arNavigationSteps = JSON.parse(
  ...
  findCurrentIndicies(
    ...
    arStepperSteps.forEach((step) => {
      ...
      if (step.steps) {
        arIndicies = this.findCurrentIndicies(
          step.steps,
          arIndicies,
          depth + 1,
        );
