...
class DataReference extends BridgeBase {
  ...
  setupDataRefUI(): any {
    // adapted from Cosmos React DX Component

    // NOTE: For now, skipped the conversion of a useEffect that's only for dropdown case...
  ...
  getDataReferenceHtml() : any {
    const dataRefHtml = html `
      <div><strong>${this.theComponentName}</strong>: component in progress
        <br /><br />
        theComponentProps:
        <br /><br />
        ${JSON.stringify(this.theComponentProps)}
        <br /><br />
        with ${this.thePConn.getChildren().length} children.
        <br /><br />
  ...
  processChildrenToRender() : any {
    ...
    this.childrenToRender.map((child, i) => {
      childrenToRenderAsComponents.push(
        // html`<div>childrenToRender[${i}]<br /><br/></div>${child.getPConnect().getComponentName()}`
        this.convertChildToComponent(child)
      );
    });
