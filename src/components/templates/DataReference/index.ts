...
class DataReference extends BridgeBase {
  ...
  connectedCallback() {
    ...
    this.allowAndPersistChangesInReviewMode =
  ...
  setupDataRefUI(): any {
    if (this.firstChildMeta.type !== "Region") {
      ...
      if (this.firstChildMeta?.type === "Dropdown") {
        this.firstChildMeta.config.datasource.source = this.rawViewMetadata
          .config?.parameters
  ...
  handleSelection = (event) => {
    ...
    if (
      this.canBeChangedInReviewMode &&
      this.thePConn.getValue("__currentPageTabViewName")
    ) {
      this.thePConn
        .getActionsApi()
        .refreshCaseView(
    ...
    if (
      ...
      PCore.getDataApiUtils()
        ...
        .then((caseResponse) => {
          const pageTokens = this.thePConn
            .getPageReference()
            .replace("caseInfo.content", "")
            .split(".");
          let curr = {};
    ...
    return inChildPConn().createComponent({
      ...
      config: {
        ...
        dataRelationshipContext:
          this.rawViewMetadata.config.contextClass &&
  ...
  processChildrenToRender(): any {
    ...
    if (this.displayMultiref) {
      return html`<multi-reference-readonly
        .pConn=${this.firstChildPConnect()}
        .hideLabel=${this.hideLabel}
        .label=${this.propsToUse.label}
      >
      </multi-reference-readonly>`;
    } else if (this.displaySingleRef) {
      return html`<single-reference-readonly
        .pConn=${this.firstChildPConnect()}
      >
  ...
  convertChildToComponent(inChild): any {
    ...
    switch (childType) {
      case "SimpleTableSelect":
        convertedComponent = html`<simple-table-select
          .pConn=${childPConn}
        ></simple-table-select>`;
        break;

      case "Dropdown":
        convertedComponent = html`<dropdown-form
          .pConn=${childPConn}
        ></dropdown-form>`;
        break;

      case "AutoComplete":
        convertedComponent = html`<autocomplete-form
          .pConn=${childPConn}
        ></autocomplete-form>`;
        break;

      case "SemanticLink":
        convertedComponent = html`<semantic-link
          .pConn=${childPConn}
        ></semantic-link>`;
        break;
      ...
      default:
        console.error(
          `----> ${this.theComponentName} convertChildToComponent unknown childType: ${childType}`,
        );
        convertedComponent = html`${this.theComponentName}
        convertChildToComponent unknown childType: ${childType}`;
        break;
    }
