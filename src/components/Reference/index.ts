...
class Reference extends BridgeBase {
  ...
  @property({ attribute: false, type: Function }) referencedViewComponent =
    null;
  ...
  updateSelf() {
    ...
    if (!this.pConn.getConfigProps) {
      if (this.pConn.getPConnect()) {
        ...
        console.error(
          `Reference component: bad pConn: ${JSON.stringify(this.pConn)}`,
        );
      }
    }

    this.resolvedConfigProps = this.pConn.resolveConfigProps(
    ...
    if (!viewMetadata) {
      ...
      console.warn(
        `View not found. getComponentConfig(): ${JSON.stringify(this.pConn.getComponentConfig())}`,
      );
      return null;
    }
    ...
    if (this.bLogging) {
      console.log(
        `Reference: about to call createComponent with pageReference: context: ${this.resolvedConfigProps["context"]}`,
      );
    }
    ...
    newCompPConnect.setInheritedConfig({
      ...referenceConfig,
      readOnly: this.resolvedConfigProps["readOnly"]
        ? this.resolvedConfigProps["readOnly"]
        : false,
      displayMode: this.resolvedConfigProps["displayMode"]
        ? this.resolvedConfigProps["displayMode"]
        : null,
    });

    if (this.bLogging) {
      console.log(
        `Web Component Reference component: newCompPConnect configProps: ${JSON.stringify(newCompPConnect.getConfigProps())}`,
      );
    }
  ...
  getComponentToRender() {
    if (this.bLogging) {
      console.log(
        `Reference: getComponentToRender: displayOnlyFA: ${this.displayOnlyFA}`,
      );
    }
    ...
    if (this.bLogging) {
      ...
      if (theRefViewComp) {
        console.log(
          `Reference: this.referencedViewComponent: ${JSON.stringify(theRefViewComp?.getConfigProps())}`,
        );
      } else {
    ...
    const viewContent = this.referencedViewComponent
      ? html`<view-component
          .pConn=${this.referencedViewComponent}
