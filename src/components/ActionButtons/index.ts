  ...
  mainButtons(): any {
    ...
    if (this.arMainButtons != null) {
      for (const aButton of this.arMainButtons) {
        ...
        arButtonHtml.push(
          html`<button class="btn btn-primary" jsAction=${jsAction} buttonType="primary" @click="${this._buttonClick}">${this.localizedVal(name, this.localeCategory)}</button>`
        );
  ...
  secondaryButtons(): any {
    ...
    if (this.arSecondaryButtons != null) {
      for (const aButton of this.arSecondaryButtons) {
        ...
        arButtonHtml.push(
          html`<button class="btn btn-secondary" jsAction=${jsAction} buttonType="secondary" @click="${this._buttonClick}">${this.localizedVal(name, this.localeCategory)}</button>`
        );
