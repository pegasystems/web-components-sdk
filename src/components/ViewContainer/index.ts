...
class ViewContainer extends BridgeBase {
  ...
  @property({ attribute: false, type: String }) title: string =
    `${this.theComponentName}: placeholder`;
  ...
  updateSelf() {
    ...
    if (this.bLogging) {
      console.log(
        `${this.theComponentName}: routingInfo prop: ${JSON.stringify(routingInfo)} | loadingInfo: ${loadingInfo}`,
      );
    }
    ...
    if (routingInfo) {
      ...
      if (accessedOrder && items) {
        ...
        if (
          ...
          if (this.bLogging) {
            console.log(
              `---> ${this.theComponentName} created new ${newCompName}`,
            );
          }
  ...
  render() {
    ...
    if (this.bLogging) {
      console.log(
        `${this.theComponentName}: render with showLoadingIndicator: ${showLoadingIndicator}`,
      );
    }
    ...
    const theCreatedRefComponent = html`<reference-component
