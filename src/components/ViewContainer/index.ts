...
class ViewContainer extends BridgeBase {
    ...
    if (routingInfo) {
      ...
      if (accessedOrder && items) {
        ...
        if (items[key] && items[key].view && Object.keys(items[key].view).length > 0) {
          ...
          if (this.bLogging) {
            console.log(`---> ${this.theComponentName} created new ${newCompName}`);
          }
  ...
  render() {
    ...
    if (this.templateName !== '') {
      theInnerTemplate = html` ${this.getTemplateForTemplate(this.templateName, this.createdViewPConn, this.displayOnlyFA)} `;
