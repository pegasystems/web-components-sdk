...
import { format } from "../../../helpers/formatters/index.js";
...
class SummaryItem extends LitElement {
  ...
  arPrimaryFieldsWithStatus: Array<any> = [];
  ...
  updatePrimaryWithStatus() {
    ...
    if (this.bShowStatus) {
      const oStatus = {
        type: "status",
        config: { value: this.status, label: "Status" },
      };
  ...
  getFieldValue(field: any): any {
    if (field.config.value == null || field.config.value == "") {
      switch (field.type.toLowerCase()) {
        case "caseoperator":
          return html`<operator-extension
            .caseOpConfig=${field.config}
          ></operator-extension>`;
        default:
    ...
    switch (field.type.toLowerCase()) {
      case "textinput":
        return html`<span class="psdk-csf-text-style"
          >${field.config.value}</span
        >`;
      case "status":
        return html`<span class="psdk-csf-status-style"
          >${field.config.value}</span
        >`;
      case "phone":
        return html`<a as="a" href="tel: ${field.config.value}"
          >${field.config.value}</a
        >`;
      case "email":
        return html`<a href="mailto: ${field.config.value}"
          >${field.config.value}</a
        >`;
      case "date":
        return html`<span class="psdk-csf-text-style"
          >${format(field.config.value, field.type)}</span
        >`;
      case "caseoperator":
        return html`<operator-extension
          .caseOpConfig=${field.config}
