      ...
      ${this.bShowAlert
        ? html` <div class="psdk-cancel-alert-background ">
            <div class="psdk-cancel-alert-top">
              ${this.heading != '' ? html`<h3>${this.heading}</h3>` : html``}
              <div>
                <p>${this.body1}</p>
                <p>${this.body2}</p>
              </div>
              <mat-grid-list cols="2" rowHeight="6.25rem">
                <mat-grid-tile>
                  <button mat-raised-button color="secondary" jsAction="save" @click="${this._buttonClick}">
                    ${this.localizedVal('Save for later', this.localeCategory)}
                  </button>
                </mat-grid-tile>
                <mat-grid-tile>
