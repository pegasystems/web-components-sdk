...
//    this.logout = this.actionsAPI.logout.bind(this.actionsAPI);
  ...
  navPanelButtonClick(oPageData: any) {
    ...
    this.thePConn.getActionsApi().showPage(pyRuleName, pyClassName).then(() => {
      if (this.bLogging) { console.log(`showPage completed`); }
    });
  ...
  navPanelCreateCaseType(sCaseType: string, sFlowType: string) {
    ...
    this.thePConn.getActionsApi().createWork(sCaseType, actionInfo).then(() => {
      if (this.bLogging) { console.log(`createWork completed`); }
    });
  ...
  render(){
    ...
    const theCaseTypeButtons = html`
      ${this.caseTypes.map((caseType) => html`<div style="display: flex;"><button class="btn btn-link text-white" 
          @click=${() => { this.navPanelCreateCaseType(caseType.pyClassName, caseType.pyFlowType)}}>${caseType.pyLabel}</button></div>`)}
    `;
    ...
    const theTemplate = html`
      <div class="psdk-appshell-nav">
          ...
          <div>
              <ul class="psdk-nav-ul-middle">
                ${this.navPages.map((page) => html`<li class="psdk-nav-li-middle">
                    <button @click=${() => { this.navPanelButtonClick(page) }} class="psdk-appshell-buttonnav" style="width: 100%; text-align: left; padding: 1rem 0rem;">
          ...
          <div class="psdk-appshell-bottom">
              <ul class="psdk-nav-ul-middle">
                  <li class="psdk-nav-li-middle">
                      <button @click=${this.navPanelOperatorButtonClick} class="psdk-appshell-buttonnav" style="width: 100%; text-align: left; padding: 1rem 0rem; margin-left: -0.75rem">
