...
class AppEntry extends LitElement {
  ...
  render() {
    if (this.bLogging) {
      console.log(
        `${this.theComponentName}: render with props - ${JSON.stringify(this.props)}`,
      );
    }
    ...
    if (this.isEmptyObject(this.props)) {
      ...
      if (this.thePConnComponentName === "RootContainer") {
        return html` <!-- <div style='width: fit-content; border: dotted 1px #DDDDDD;'>${this
            .theComponentName}</div>  -->
          <root-container .pConn=${this.props}></root-container>`;
      } else {
        if (this.bLogging) {
          console.log(
            `${this.theComponentName} not known as AppEntry rendered component. So rendering NotSupported`,
          );
        }
        return html`
          <not-supported
            >AppEntry got ${this.theComponentName} to render</not-supported
          >
        `;
      }
    }
