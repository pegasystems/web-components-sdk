...
class Trade extends LitElement {
  static styles?: CSSResultGroup = tradeStyles;
  ...
  getStarted() {
    ...
    PCore.getMashupApi()
      .createCase('O533RU-UplusAuto-Work-TradeIn', this.pConn.getContextName())
      .then(() => {
      ...
      mMSHtml = html`
        <div class="cc-resolution">
          <div class="cc-body-uplus">
            <div style="font-size: 22px;">Congrats on your trade in offer!</div>
            <div
              style="font-size: 18px;
          margin-top: 16px;"
            >
              We will contact you within 24 hours to schedule pick up of the vehicle. Until then, you’re all set!
            </div>
          </div>
        </div>
      `;
    } else {
      mMSHtml = html` ${this.showTradeIn
        ? html`
